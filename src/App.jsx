...
function buildVolumeProfile(bars, tick) {
  ...
  return { poc, vah: Math.max(vah, poc), val: Math.min(val, poc), entries };
}
...
function calcSwingLevels(bars) {
  ...
  return { swingHigh: Math.max(...recent.map((b) => b.high)), swingLow: Math.min(...recent.map((b) => b.low)) };
}
...
function generateSignal(bars, vp) {
  ...
  return {
    side, entry,
    sl: parseFloat(sl.toFixed(2)), tp: parseFloat(tp.toFixed(2)), rr: parseFloat(rr.toFixed(2)),
    confidence: side === "LONG" ? longScore : shortScore,
    longScore, shortScore,
    reasons: reasons.filter((r) => r.side === side.toLowerCase()),
    atr: parseFloat(atr.toFixed(2)), absorption, cumulDelta: parseFloat(cumulDelta.toFixed(0)),
...
function FootprintBar({ bar, isLast, tick, decimals }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1px", background: isLast ? "rgba(0,255,170,0.04)" : "transparent", border: isLast ? "1px solid rgba(0,255,170,0.2)" : "1px solid transparent", borderRadius: 4, padding: "2px", minWidth: 90 }}>
      {levels.map((l, idx) => (
        <div key={idx} style={{ display: "flex", gap: 2, alignItems: "center", height: 14 }}>
          <span style={{ fontSize: 8, color: "#0ff", width: 46, textAlign: "right", opacity: 0.6 }}>{l.price.toFixed(decimals)}</span>
          ...
          <span style={{ fontSize: 7, color: bar.delta > 0 ? "#0f9" : "#f55", opacity: 0.5 }}>{bar.delta > 0 ? "+" : ""}{Math.round(bar.delta / 10)}</span>
        </div>
      ))}
      <div style={{ textAlign: "center", fontSize: 8, marginTop: 2, color: bar.delta > 0 ? "#0f9" : "#f55" }}>Δ {bar.delta > 0 ? "+" : ""}{bar.delta}</div>
    </div>
  );
}
...
function VolumeProfileBar({ entry, max, poc, vah, val, tick }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 4, height: 10 }}>
      <div style={{ width: `${width}%`, height: 8, borderRadius: 1, background: isPOC ? "#ffe44d" : isVAH ? "#ff6b6b" : isVAL ? "#4dffa0" : "rgba(100,180,255,0.4)", transition: "width 0.3s" }} />
    </div>
  );
}
    ...
    <div style={{ background: bgColor, border: `1px solid ${borderColor}`, borderRadius: 12, padding: "16px 20px", boxShadow: `0 0 30px ${isLong ? "rgba(0,255,170,0.1)" : "rgba(255,77,109,0.1)"}` }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ background: color, color: "#000", fontWeight: 800, fontSize: 14, padding: "4px 12px", borderRadius: 6 }}>{isLong ? "▲ LONG" : "▼ SHORT"}</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
        {[{ label: "ENTRY", value: signal.entry?.toFixed(2), color: "#fff" }, { label: "TAKE PROFIT", value: signal.tp?.toFixed(2), color: "#00ffaa" }, { label: "STOP LOSS", value: signal.sl?.toFixed(2), color: "#ff4d6d" }].map((item) => (
...
function makeMonitor() {
  return { bars: [], vp: null, signal: null, openPosition: null, signalHistory: [] };
}
...
export default function Dashboard() {
  ...
  // display state — only the selected symbol drives re-renders
  const [display, setDisplay]           = useState({ bars: [], vp: null, signal: null, signalHistory: [], currentPrice: null });

  const monitorsRef    = useRef(Object.fromEntries(SYMBOLS.map((s) => [s.id, makeMonitor()])));
  const selectedIdRef  = useRef(selectedId);
  const wsRef          = useRef(null);

  // Keep ref in sync so the WS handler (closure) always reads the latest selectedId
  useEffect(() => { selectedIdRef.current = selectedId; }, [selectedId]);

  const refreshDisplay = useCallback((symbolId) => {
    const mon = monitorsRef.current[symbolId];
    setDisplay({
      bars:          [...mon.bars],
      vp:            mon.vp,
      signal:        mon.signal,
      signalHistory: [...mon.signalHistory],
      currentPrice:  mon.bars[mon.bars.length - 1]?.close || null,
    });
  }, []);

  // Fetch initial bars for ALL symbols
  useEffect(() => {
    SYMBOLS.forEach(async (sym) => {
      try {
        const res    = await fetch(`https://api.binance.com/api/v3/klines?symbol=${sym.id}&interval=${INTERVAL}&limit=${BAR_LIMIT}`);
        ...
        const mon = monitorsRef.current[sym.id];
        mon.bars  = parsed;
        if (parsed.length >= 10) {
          mon.vp     = buildVolumeProfile(parsed.slice(-20), sym.tick);
          mon.signal = generateSignal(parsed, mon.vp);
        }
        setPrices((prev) => ({ ...prev, [sym.id]: parsed[parsed.length - 1]?.close }));
        if (sym.id === selectedIdRef.current) refreshDisplay(sym.id);
      } catch (e) { console.error("Fetch error", sym.id, e); }
    });
  }, [refreshDisplay]);

  // Single combined WebSocket for all 3 symbols
  useEffect(() => {
    const streams = SYMBOLS.map((s) => `${s.id.toLowerCase()}@kline_${INTERVAL}`).join("/");
    ...
    ws.onmessage = (evt) => {
      ...
      // Identify which symbol this kline belongs to
      const sym = SYMBOLS.find((s) => stream.startsWith(s.id.toLowerCase()));
      if (!sym) return;
      ...
      // ── TP/SL resolution ──────────────────────────────────────────────────
      if (mon.openPosition) {
        const pos   = mon.openPosition;
        const hitTP = pos.side === "LONG" ? bar.close >= pos.tp : bar.close <= pos.tp;
        const hitSL = pos.side === "LONG" ? bar.close <= pos.sl : bar.close >= pos.sl;
        if (hitTP || hitSL) {
          const outcome  = hitTP ? "WIN" : "LOSS";
          mon.signalHistory = mon.signalHistory.map((e) =>
            e.id === pos.id ? { ...e, outcome, closePrice: bar.close } : e
          );
          sendTelegram(buildCloseMessage(pos, outcome, bar.close, sym.label));
          mon.openPosition = null;
        }
      }

      // ── Signal generation ─────────────────────────────────────────────────
      if (current.length >= 10) {
        mon.vp     = buildVolumeProfile(current.slice(-20), sym.tick);
        mon.signal = generateSignal(current, mon.vp);

        if (mon.signal?.side && !mon.openPosition) {
          const id  = Date.now();
          mon.openPosition = { id, side: mon.signal.side, entry: mon.signal.entry, tp: mon.signal.tp, sl: mon.signal.sl };
          const newEntry   = { ...mon.signal, id, time: new Date().toLocaleTimeString(), outcome: null, symbol: sym.label };
          mon.signalHistory = [...mon.signalHistory.slice(-19), newEntry];
          sendTelegram(buildOpenMessage(mon.signal, sym.label));
        }
      }

      // ── Update prices ticker ──────────────────────────────────────────────
      setPrices((prev) => ({ ...prev, [sym.id]: bar.close }));
      setLastUpdate(new Date().toLocaleTimeString());

      // ── Update display only for selected symbol ───────────────────────────
      if (sym.id === selectedIdRef.current) refreshDisplay(sym.id);
    };
  ...
  // When user switches symbol, immediately repaint from cached monitor data
  const handleSelectSymbol = useCallback((symId) => {
    setSelectedId(symId);
    selectedIdRef.current = symId;
    refreshDisplay(symId);
  }, [refreshDisplay]);
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {SYMBOLS.map((s) => {
              ...
              const mon = monitorsRef.current[s.id];
              const hasSig = mon?.signal?.side;
              return (
                <button key={s.id} onClick={() => handleSelectSymbol(s.id)} style={{
                  background: isActive ? "rgba(0,255,170,0.15)" : "transparent",
                  border: `1px solid ${isActive ? "rgba(0,255,170,0.5)" : hasSig ? "rgba(255,220,50,0.4)" : "rgba(255,255,255,0.1)"}`,
                  color: isActive ? "#00ffaa" : hasSig ? "#ffe44d" : "#666",
                  fontFamily: "inherit", fontSize: 12, padding: "3px 10px",
                  borderRadius: 6, cursor: "pointer", fontWeight: isActive ? 700 : 400,
                  transition: "all 0.2s", position: "relative"
                }}>
                  {s.label}
                  {hasSig && !isActive && (
                    <span style={{ position: "absolute", top: -3, right: -3, width: 7, height: 7, borderRadius: "50%", background: hasSig === "LONG" ? "#00ffaa" : "#ff4d6d", animation: "pulse 1s infinite" }} />
                  )}
                </button>
              );
            })}
          </div>
          <span style={{ color: "#444", fontSize: 12 }}>· 5m · all monitored</span>
        </div>

        {/* All 3 prices in header */}
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          {SYMBOLS.map((s) => (
            <div key={s.id} style={{ textAlign: "right" }}>
              <div style={{ color: "#444", fontSize: 9 }}>{s.label}</div>
              <div style={{ color: s.id === selectedId ? "#fff" : "#666", fontSize: s.id === selectedId ? 16 : 12, fontWeight: 700 }}>
                {prices[s.id] ? `$${prices[s.id].toFixed(s.decimals)}` : "—"}
              </div>
            </div>
          ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 300px", height: "calc(100vh - 52px)" }}>
        ...
        <div style={{ overflow: "auto", padding: 16 }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 16, marginBottom: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <span style={{ color: "#888", fontSize: 11, letterSpacing: 2 }}>FOOTPRINT · VOLUME PROFILE · {symbolLabel}</span>
              <div style={{ display: "flex", gap: 12, fontSize: 10 }}>
                <span style={{ color: "#ffe44d" }}>● POC</span><span style={{ color: "#ff6b6b" }}>● VAH</span><span style={{ color: "#4dffa0" }}>● VAL</span>
              </div>
            </div>
            {vp && (
              <div style={{ marginBottom: 12 }}>
                {[...vp.entries].sort((a, b) => b.price - a.price).slice(0, 25).map((e, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 1 }}>
                    <span style={{ fontSize: 8, width: 58, textAlign: "right", fontFamily: "monospace", color: Math.abs(e.price - vp.poc) < tick ? "#ffe44d" : Math.abs(e.price - vp.vah) < tick * 2 ? "#ff6b6b" : Math.abs(e.price - vp.val) < tick * 2 ? "#4dffa0" : "#555" }}>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 16, marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 3, alignItems: "flex-end", height: 80 }}>
              {displayBars.map((bar, i) => {
                const maxD = Math.max(...displayBars.map((b) => Math.abs(b.delta)), 1);
                const h    = Math.abs(bar.delta) / maxD * 70;
                return (
            ...
            <div style={{ display: "flex", gap: 3, height: 60, alignItems: "flex-start" }}>
              {displayBars.map((bar, i) => {
                const maxD = Math.max(...displayBars.map((b) => Math.abs(b.delta)), 1);
                const h    = bar.delta < 0 ? Math.abs(bar.delta) / maxD * 55 : 0;
                return <div key={i} style={{ flex: 1 }}>{h > 0 && <div style={{ height: h, width: "100%", minHeight: 2, background: `rgba(255,77,109,${0.4+(h/55)*0.6})`, borderRadius: "0 0 2px 2px" }} />}</div>;
              })}
            </div>
          </div>

          {/* Signal History — all symbols combined */}
            ...
            return (
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  <span style={{ color: "#888", fontSize: 11, letterSpacing: 2 }}>SIGNAL HISTORY · {symbolLabel}</span>
                  {winrate !== null && (
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[...signalHistory].reverse().map((s, i) => {
                    ...
                    return (
                      <div key={s.id ?? i} style={{ display: "grid", gridTemplateColumns: "70px 1fr 1fr 1fr 52px 52px", alignItems: "center", gap: 6, padding: "7px 10px", borderRadius: 6, background: isOpen ? (s.side === "LONG" ? "rgba(0,255,170,0.08)" : "rgba(255,77,109,0.08)") : "rgba(255,255,255,0.02)", border: `1px solid ${isOpen ? (s.side === "LONG" ? "rgba(0,255,170,0.3)" : "rgba(255,77,109,0.3)") : "rgba(255,255,255,0.06)"}`, fontSize: 11 }}>
                        ...
                        <span style={{ color: outcomeColor, fontWeight: 700, textAlign: "right", animation: isOpen ? "pulse 1.5s infinite" : "none" }}>{s.outcome ?? "OPEN"}</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <div style={{ borderLeft: "1px solid rgba(255,255,255,0.07)", padding: 16, display: "flex", flexDirection: "column", gap: 12, overflowY: "auto" }}>
          <div style={{ color: "#888", fontSize: 11, letterSpacing: 2 }}>LIVE SIGNAL · {symbolLabel}</div>
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 14 }}>
            ...
            {[
              ...
              { label: "Absorption",     value: signal?.absorption ? "DETECTED" : "No", color: signal?.absorption ? "#ffe44d" : "#555" },
          ...
          {(() => {
            ...
            return (
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 12, padding: 14 }}>
                <div style={{ color: "#888", fontSize: 11, letterSpacing: 2, marginBottom: 10 }}>RESULTS · {symbolLabel}</div>
                <div style={{ textAlign: "center", marginBottom: 12 }}>
                  <div style={{ fontSize: 28, fontWeight: 800, color: winrate === null ? "#444" : winrate >= 50 ? "#00ffaa" : "#ff4d6d" }}>{winrate !== null ? `${winrate}%` : "—"}</div>
                ...
                {openPos ? (
                  <div style={{ background: openPos.side === "LONG" ? "rgba(0,255,170,0.08)" : "rgba(255,77,109,0.08)", border: `1px solid ${openPos.side === "LONG" ? "rgba(0,255,170,0.3)" : "rgba(255,77,109,0.3)"}`, borderRadius: 8, padding: "8px 10px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <span style={{ color: openPos.side === "LONG" ? "#00ffaa" : "#ff4d6d", fontWeight: 700, fontSize: 12 }}>{openPos.side === "LONG" ? "▲" : "▼"} {openPos.side} — OPEN</span>
                      <span style={{ color: "#ffe44d", animation: "pulse 1.5s infinite" }}>●</span>
                    </div>
                    {[{ label: "Entry", value: `$${openPos.entry?.toFixed(2)}`, color: "#fff" }, { label: "TP", value: `$${openPos.tp?.toFixed(2)}`, color: "#00ffaa" }, { label: "SL", value: `$${openPos.sl?.toFixed(2)}`, color: "#ff4d6d" }].map((row) => (
