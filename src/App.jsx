...
// ─── Helpers ─────────────────────────────────────────────────────────────────
const roundToTick = (price) => Math.round(price / TICK_SIZE) * TICK_SIZE;
...
function buildVolumeProfile(bars) {
  ...
  bars.forEach((b) => {
    ...
    for (let i = 0; i < LEVELS_PER_BAR; i++) {
      const price = roundToTick(b.low + step * i);
...
function isLiquidSession() {
  // Only trade 07:00–23:00 UTC (London + NY sessions)
...
function generateSignal(bars, vp) {
  ...
  // ── Hard filters (no signal at all if these fail) ──────────────────────────
  if (!last.closed) return empty;                          // wait for bar close
  if (!isLiquidSession()) return empty;                    // skip Asian dead zone
  const pocDist = Math.abs(last.close - vp.poc) / (vp.poc || 1);
  if (pocDist < 0.002) return empty;                       // too close to fair value
  ...
  // ── LONG conditions (8 total, need 5) ──────────────────────────────────────
  ...
  // ── SHORT conditions (8 total, need 5) ────────────────────────────────────
  ...
  return {
    ...
    absorption,
    cumulDelta: parseFloat(cumulDelta.toFixed(0)),
    highVolume,
    stackedImbalance: side === "LONG" ? stackedBull : stackedBear,
  };
}

// ─── Sub-components ───────────────────────────────────────────────────────────

function FootprintBar({ bar, maxVol, isLast }) {
  if (!bar) return null;
  const step = (bar.high - bar.low) / 10 || 0.5;
  const levels = [];
  for (let i = 9; i >= 0; i--) {
    const price = roundToTick(bar.low + step * i);
    ...
    <div style={{
      ...
      {levels.map((l, idx) => (
        <div key={idx} style={{ display: "flex", gap: 2, alignItems: "center", height: 14 }}>
          <span style={{ fontSize: 8, color: "#0ff", width: 42, textAlign: "right", opacity: 0.6 }}>
            {l.price.toFixed(1)}
          </span>
          <div style={{
            background: `rgba(0,200,120,${Math.min(l.buyVol / 200, 0.9)})`,
            width: Math.max(l.buyVol / 8, 4), height: 10, borderRadius: 1
          }} />
          <div style={{
            background: `rgba(255,80,80,${Math.min(l.sellVol / 200, 0.9)})`,
            width: Math.max(l.sellVol / 8, 4), height: 10, borderRadius: 1
          }} />
      ...
      <div style={{
        textAlign: "center", fontSize: 8, marginTop: 2,
        color: bar.delta > 0 ? "#0f9" : "#f55",
...
function VolumeProfileBar({ entry, max, poc, vah, val }) {
  const width = Math.max((entry.vol / max) * 100, 2);
  const isPOC = Math.abs(entry.price - poc) < TICK_SIZE;
  const isVAH = Math.abs(entry.price - vah) < TICK_SIZE * 2;
  const isVAL = Math.abs(entry.price - val) < TICK_SIZE * 2;
  return (
...
function SignalCard({ signal }) {
    ...
    return (
      <div style={{
        ...
        <div style={{ color: "#555", fontSize: 11, marginTop: 6 }}>
          Long: {signal.longScore}/5 • Short: {signal.shortScore}/5
        </div>
        <div style={{ color: "#444", fontSize: 10, marginTop: 4 }}>Need 3+ conditions to trigger</div>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      boxShadow: `0 0 30px ${isLong ? "rgba(0,255,170,0.1)" : "rgba(255,77,109,0.1)"}`
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ color: "#888", fontSize: 11 }}>ETH/USDT</div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ color: "#888", fontSize: 10 }}>CONFIDENCE</div>
          <div style={{ color: color, fontWeight: 700, fontSize: 18 }}>{signal.confidence}/8</div>
        </div>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 12 }}>
        {[
          ...
          <div key={item.label} style={{
            background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: "8px 10px", textAlign: "center"
          }}>
            <div style={{ color: "#555", fontSize: 9, letterSpacing: 1 }}>{item.label}</div>
            <div style={{ color: item.color, fontFamily: "monospace", fontSize: 13, fontWeight: 700, marginTop: 2 }}>
              ${item.value}
            </div>
          </div>
        ))}
      ...
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10, fontSize: 11 }}>
        <span style={{ color: "#888" }}>R:R Ratio: <span style={{ color: "#ffe44d", fontWeight: 700 }}>1:{signal.rr}</span></span>
...
export default function Dashboard() {
  ...
  const openPositionRef = useRef(null); // null | { id, side, entry, tp, sl }

  const fetchInitialBars = useCallback(async () => {
    try {
      const res = await fetch(
        `https://api.binance.com/api/v3/klines?symbol=${SYMBOL}&interval=${INTERVAL}&limit=${BAR_LIMIT}`
      );
  ...
  useEffect(() => {
    ...
    ws.onmessage = (evt) => {
      ...
      if (stream.includes("kline")) {
        ...
        if (lastIdx >= 0) current[lastIdx] = bar;
        else current.push(bar);
  ...
  // Recalculate VP and signal whenever bars change
  useEffect(() => {
    if (bars.length < 10) return;
    const vpData = buildVolumeProfile(bars.slice(-20));
    setVP(vpData);
    ...
    if (sig?.side && !openPositionRef.current) {
      ...
      const newEntry = { ...sig, id, time: new Date().toLocaleTimeString(), outcome: null };
      setSignalHistory((prev) => [...prev.slice(-19), newEntry]);
    }
    // position only closes when TP/SL is hit — not when score drops
  }, [bars]);

  // Resolve open position when TP or SL is hit
  useEffect(() => {
    ...
    if (hitTP || hitSL) {
      ...
      setSignalHistory((prev) =>
        prev.map((e) => e.id === pos.id ? { ...e, outcome, closePrice: currentPrice } : e)
      );
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            background: connected ? "#00ffaa" : "#ff4d6d",
            boxShadow: connected ? "0 0 8px #00ffaa" : "0 0 8px #ff4d6d"
          }} />
          <span style={{ color: "#00ffaa", fontWeight: 700, fontSize: 15, letterSpacing: 2 }}>FLOW</span>
          <span style={{ color: "#444", fontSize: 13 }}>|</span>
          <span style={{ color: "#aaa", fontSize: 13 }}>ETH/USDT · 5m</span>
        </div>
                ...
                {[...vp.entries]
                  ...
                  .map((e, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 1 }}>
                      <span style={{
                        fontSize: 8, width: 55, textAlign: "right", fontFamily: "monospace",
                        color: Math.abs(e.price - vp.poc) < TICK_SIZE ? "#ffe44d"
                          : Math.abs(e.price - vp.vah) < TICK_SIZE * 2 ? "#ff6b6b"
                            : Math.abs(e.price - vp.val) < TICK_SIZE * 2 ? "#4dffa0"
                              : "#555"
                      }}>
                        {e.price.toFixed(1)}
                      </span>
                      <VolumeProfileBar entry={e} max={vpMax} poc={vp.poc} vah={vp.vah} val={vp.val} />
                      <span style={{ fontSize: 7, color: "#444" }}>{Math.round(e.vol)}</span>
                    </div>
                  ))}
                <div style={{ display: "flex", gap: 16, marginTop: 8, fontSize: 10 }}>
                  <span style={{ color: "#ffe44d" }}>POC: ${vp.poc.toFixed(2)}</span>
                  <span style={{ color: "#ff6b6b" }}>VAH: ${vp.vah.toFixed(2)}</span>
                  <span style={{ color: "#4dffa0" }}>VAL: ${vp.val.toFixed(2)}</span>
                </div>
              </div>
            )}
              ...
              <div style={{ display: "flex", gap: 4, alignItems: "flex-end", minWidth: "fit-content" }}>
                {displayBars.map((bar, i) => (
                  <div key={bar.time} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                    <FootprintBar
                      bar={bar}
                      maxVol={Math.max(...displayBars.map((b) => b.volume))}
                      isLast={i === displayBars.length - 1}
                    />
                    <div style={{
                  ...
                  <div key={i} style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1 }}>
                    {bar.delta > 0 ? (
                      <>
                        ...
                        <div style={{
                          height: h, width: "100%", minHeight: 2,
                          background: `rgba(0,255,170,${0.4 + (h / 70) * 0.6})`,
                      ...
                      <>
                        ...
                        <div style={{
                          height: h, width: "100%", minHeight: 2,
                          background: `rgba(255,77,109,${0.4 + (h / 70) * 0.6})`,
                          borderRadius: "2px 2px 0 0"
                        }} />
                      </>
                    )}
                ...
                return (
                  <div key={i} style={{ flex: 1 }}>
                    {h > 0 && (
                      <div style={{
                        height: h, width: "100%", minHeight: 2,
                        background: `rgba(255,77,109,${0.4 + (h / 55) * 0.6})`,
              ...
              <div style={{
                ...
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                  ...
                  {winrate !== null && (
                    <div style={{ display: "flex", gap: 12, fontSize: 11 }}>
                      ...
                      <span style={{
                        color: winrate >= 50 ? "#00ffaa" : "#ff4d6d",
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[...signalHistory].reverse().map((s, i) => {
                    ...
                    const outcomeLabel = s.outcome ?? "OPEN";
                    return (
                      <div key={s.id ?? i} style={{
                        display: "grid",
                        gridTemplateColumns: "80px 1fr 1fr 1fr 60px 56px",
                        alignItems: "center", gap: 6,
                        padding: "7px 10px", borderRadius: 6,
                        background: isOpen
                          ? (s.side === "LONG" ? "rgba(0,255,170,0.08)" : "rgba(255,77,109,0.08)")
                          : "rgba(255,255,255,0.02)",
                        border: `1px solid ${isOpen
                          ? (s.side === "LONG" ? "rgba(0,255,170,0.3)" : "rgba(255,77,109,0.3)")
                          : "rgba(255,255,255,0.06)"}`,
                        fontSize: 11
                      }}>
                        <span style={{ color: s.side === "LONG" ? "#00ffaa" : "#ff4d6d", fontWeight: 700 }}>
                          {s.side === "LONG" ? "▲" : "▼"} {s.side}
                        </span>
                        ...
                        <span style={{
                          color: outcomeColor, fontWeight: 700, textAlign: "right",
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              { label: "Cum. Delta (5)", value: signal?.cumulDelta !== undefined ? signal.cumulDelta : "—", color: signal?.cumulDelta > 0 ? "#0f9" : "#f55" },
              { label: "Absorption", value: signal?.absorption ? "DETECTED" : "None", color: signal?.absorption ? "#ffe44d" : "#555" },
              { label: "POC", value: vp ? `$${vp.poc.toFixed(2)}` : "—", color: "#ffe44d" },
              { label: "VAH", value: vp ? `$${vp.vah.toFixed(2)}` : "—", color: "#ff6b6b" },
              { label: "VAL", value: vp ? `$${vp.val.toFixed(2)}` : "—", color: "#4dffa0" },
            ].map((item) => (
              <div key={item.label} style={{
            ...
            <div style={{
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                <div style={{ flex: 1, textAlign: "center" }}>
                  <div style={{ color: "#00ffaa", fontSize: 22, fontWeight: 800 }}>
                    {signal.longScore}<span style={{ fontSize: 12, color: "#333" }}>/8</span>
                  </div>
                  <div style={{ color: "#555", fontSize: 10 }}>LONG</div>
                </div>
                <div style={{ width: 1, background: "rgba(255,255,255,0.08)" }} />
                <div style={{ flex: 1, textAlign: "center" }}>
                  <div style={{ color: "#ff4d6d", fontSize: 22, fontWeight: 800 }}>
                    {signal.shortScore}<span style={{ fontSize: 12, color: "#333" }}>/8</span>
                  </div>
                  <div style={{ color: "#555", fontSize: 10 }}>SHORT</div>
                </div>
              </div>
              <div style={{ fontSize: 10, color: "#444", textAlign: "center" }}>
                5+ conditions · R:R ≥ 1.5 · closed bar
              </div>
            </div>
          )}
              ...
              <div style={{
                ...
                <div style={{ textAlign: "center", marginBottom: 12 }}>
                  <div style={{
                    ...
                    color: winrate === null ? "#444" : winrate >= 50 ? "#00ffaa" : "#ff4d6d"
                  }}>
                ...
                {openPos ? (
                  <div style={{
                    background: openPos.side === "LONG" ? "rgba(0,255,170,0.08)" : "rgba(255,77,109,0.08)",
                    border: `1px solid ${openPos.side === "LONG" ? "rgba(0,255,170,0.3)" : "rgba(255,77,109,0.3)"}`,
